...
/**
 * Current User Email Compromised Version
 */
if ( ! defined( 'ECC_VERSION' ) ) {
	define( 'ECC_VERSION', '1.0' );
}
/** *
 * Define our Email Comrpomised Class
 *
 * @package User Email Comrpomised Check
 * @since   4.2
 */
class Email_Comrpomised_Check {
	...
	public function ecc_admin_notice() {
		if ( isset( $_GET['action'] ) && 'pwned' === $_GET['action'] ) {
			if ( isset( $_GET['users'] ) ) {
				$hidden_array = array();
				...
				?>
				<div class="updated">
					<p><?php esc_attr_e( 'The following users have been crosschecked with the Have I Been Pwned API', 'my-text-domain' ); ?>:</p>
					<ul>
						<?php
						$i = 1;
						...
						if ( ! empty( $hidden_array ) ) {
							?>
							<section class="hidden-returned-data">
							<?php
	...
	*/
	public function ecc_get_user_email() {
		global $pagenow;
		if ( 'profile.php' === $pagenow ) { // viewing your current profile
			global $current_user;
			get_currentuserinfo();
			return $current_user->user_email;
		} else if ( 'user-edit.php' === $pagenow ) { // editing another user
	...
	*/
	public function ecc_has_email_been_pwned( $email = '' ) {
		...
		if ( $this->ecc_is_user_profile_page() ) {
			...
			if ( '404' === $response_code ) { // account is clean
		...
		?>
			<script type="text/javascript">
			  jQuery(document).ready(function() {
				jQuery('<option>').val('pwned').text( '<?php esc_attr_e( 'Is It Pwned?', 'ecc-domain' ); ?>' ).appendTo("select[name='action']");
			  });
			</script>
		<?php
	}
